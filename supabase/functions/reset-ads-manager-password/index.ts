// Supabase Edge Function: reset-ads-manager-password
// Bob calls this to set a new password for any Ads Manager user.
// Deploy: supabase functions deploy reset-ads-manager-password
...
serve(async (req) => {
  ...
  try {
    ...
    if (callerProfile?.role !== 'bob') {
      return json({ error: 'Forbidden — only Bob can reset Ads Manager passwords' }, 403);
    }
    ...
    if (targetProfile.role !== 'ads_manager') {
      return json({ error: 'Can only reset Ads Manager passwords via this endpoint' }, 403);
    }
