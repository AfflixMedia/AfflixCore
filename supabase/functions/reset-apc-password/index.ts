...
serve(async (req) => {
  ...
  try {
    ...
    if (callerProfile?.role !== 'bob') return json({ error: 'Forbidden — only Bob can reset APC passwords' }, 403);
    ...
    // 3. Confirm target is an APC (defense in depth — Bob can't accidentally reset another Bob)
