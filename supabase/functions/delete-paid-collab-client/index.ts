// Supabase Edge Function: delete-paid-collab-client
// Bob calls this to permanently remove a Paid Collab Client account.
// Deploy in dashboard: Edge Functions → Create → name "delete-paid-collab-client"
...
serve(async (req) => {
  ...
  try {
    ...
    if (callerProfile?.role !== 'bob') return json({ error: 'Forbidden — only Bob can delete Paid Collab Clients' }, 403);
    ...
    if (targetProfile.role !== 'paid_collab_client') {
      return json({ error: 'Can only delete Paid Collab Client accounts via this endpoint' }, 403);
    }
