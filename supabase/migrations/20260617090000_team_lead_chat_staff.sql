-- =========================================================
-- Afflix Core — Team Lead in Global Chat (Phase 1d)
--
-- The chat staff roster is hard-coded as role in ('bob','apc','paid_collab_handler')
-- in several SECURITY DEFINER functions. Re-create the latest version of each with
-- 'team_lead' added so Team Leads appear as contacts, can be DM'd / added to groups,
-- and receive announcement + @-mention notifications.
--
-- Bodies are reproduced verbatim from their latest definitions; the ONLY change is
-- the role list. is_internal_staff() already includes team_lead (see
-- 20260616090000_team_lead_role.sql).
-- =========================================================

-- ---------- chat_list_contacts (from 20260608) ----------
...
as $$
  ...
  where p.role in ('bob', 'team_lead', 'apc', 'paid_collab_handler')
    and p.id <> auth.uid()
...
-- ---------- chat_get_or_create_dm (from 20260608) ----------
...
begin
  ...
  if not exists (
    ...
    where id = other_user and role in ('bob', 'team_lead', 'apc', 'paid_collab_handler')
  ) then
...
-- ---------- chat_create_group (from 20260609) ----------
...
begin
  ...
  foreach m in array coalesce(p_members, '{}') loop
    if m <> me and exists (select 1 from public.profiles
                           where id = m and role in ('bob','team_lead','apc','paid_collab_handler')) then
...
-- ---------- chat_add_member (3-arg, from 20260612) ----------
...
begin
  ...
  if not exists (select 1 from public.profiles where id = p_user and role in ('bob','team_lead','apc','paid_collab_handler')) then
...
-- ---------- chat_on_new_message (from 20260613) ----------
...
begin
  ...
  if is_ann then
    ...
    if new.mentions is not null and array_length(new.mentions, 1) > 0 then
      ...
      join public.profiles p on p.id = u and p.role in ('bob','team_lead','apc','paid_collab_handler')
    ...
    from public.profiles p
    where p.role in ('bob','team_lead','apc','paid_collab_handler') and p.id <> new.sender_id
